...
import { Plus, Book, FileText, Video, Link, Calendar, AlertTriangle } from 'lucide-react';
...
const MaterialManager: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!materialToAdd.max_purchase_date) delete materialToAdd.max_purchase_date;
    addMaterial(materialToAdd);
    setIsAdding(false);
    setNewMaterial({ title: '', subject_id: '', type: 'libro', total_units: 0, completed_units: 0, max_purchase_date: '' });
  };
      ...
      {isAdding && (
        <div className={`p-10 rounded-[3rem] shadow-2xl border-2 mb-12 animate-in slide-in-from-top duration-500 ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-black uppercase tracking-[0.2em] text-amber-500 mb-3">
                Fecha Máxima de Compra <span className="text-slate-400 normal-case font-normal">(opcional — libros / proyectos)</span>
              </label>
              <input
                type="date"
                value={newMaterial.max_purchase_date}
                onChange={e => setNewMaterial({...newMaterial, max_purchase_date: e.target.value})}
                className={`w-full p-5 rounded-2xl outline-none focus:ring-4 focus:ring-amber-500/20 font-bold text-lg transition-all ${theme === 'dark' ? 'bg-slate-700 text-white' : 'bg-slate-50 text-slate-900'}`}
              />
            </div>
          ...
          <div key={subject.id} className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {subjectMaterials.map(material => {
                ...
                // Calcular alerta de fecha máxima de compra
                const today = new Date();
                today.setHours(0, 0, 0, 0);
                const purchaseDate = material.max_purchase_date ? new Date(material.max_purchase_date) : null;
                const daysUntilPurchase = purchaseDate ? Math.ceil((purchaseDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : null;
                const isPurchaseOverdue = daysUntilPurchase !== null && daysUntilPurchase < 0;
                const isPurchaseSoon = daysUntilPurchase !== null && daysUntilPurchase >= 0 && daysUntilPurchase <= 7;

                return (
                  <div key={material.id} className={`group p-8 rounded-[2.5rem] border shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 ${
                    isPurchaseOverdue
                      ? 'border-red-400 ' + (theme === 'dark' ? 'bg-slate-800' : 'bg-white')
                      : isPurchaseSoon
                      ? 'border-amber-400 ' + (theme === 'dark' ? 'bg-slate-800' : 'bg-white')
                    ...
                    {material.max_purchase_date && (
                      <div className={`flex items-center gap-2 mb-6 px-3 py-2 rounded-xl text-xs font-black ${
                        isPurchaseOverdue
                        ...
                        {isPurchaseOverdue ? <AlertTriangle size={12} /> : <Calendar size={12} />}
                        <span>
                          {isPurchaseOverdue
                            ? `Compra vencida hace ${Math.abs(daysUntilPurchase!)} día${Math.abs(daysUntilPurchase!) !== 1 ? 's' : ''}`
                            : isPurchaseSoon
                            ? `Comprar en ${daysUntilPurchase} día${daysUntilPurchase !== 1 ? 's' : ''}`
                            : `Comprar antes: ${new Date(material.max_purchase_date).toLocaleDateString('es-ES')}`
                          }
                        </span>
                      </div>
                    )}
